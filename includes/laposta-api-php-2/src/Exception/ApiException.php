...
class ApiException extends \LapostaApi\Exception\LapostaException
{
     ...
     */
    public function __construct(string $message, protected RequestInterface $request, protected ResponseInterface $response, int $code = 0, ?Throwable $previous = null)
    {
    ...
    public function getResponseData() : array
    {
        if ($this->responseJson === null) {
            ...
            if (!empty($body)) {
                try {
                    $this->responseJson = \json_decode($body, \true, 512, \JSON_THROW_ON_ERROR);
