...
abstract class BaseApi
{
     ...
     */
    protected function sendRequest(string $method, array $pathSegments = [], array $queryParams = [], ?array $body = null, ContentType $contentType = ContentType::FORM) : array
    {
     ...
     * @throws \JsonException
     */
    protected function createRequest(string $method, string $url, ?array $body = null, ContentType $contentType = ContentType::FORM) : RequestInterface
    {
        // Create the request via the request factory
        $request = $this->laposta->getRequestFactory()->createRequest($method, $url);
        // Define default headers
        $defaultHeaders = ['Authorization' => 'Basic ' . \base64_encode($this->laposta->getApiKey() . ':'), 'User-Agent' => 'laposta-php-' . Laposta::VERSION, 'X-Laposta-Client-User-Agent' => \json_encode(['bindings_version' => Laposta::VERSION, 'lang' => 'php', 'lang_version' => \phpversion(), 'uname' => \php_uname()])];
        // Add default headers
    ...
    protected function handleResponse(RequestInterface $request, ResponseInterface $response) : array
    {
        ...
        if ($statusCode < 200 || $statusCode >= 300) {
            ...
            try {
                $responseData = \json_decode($body, \true, 512, \JSON_THROW_ON_ERROR);
            ...
            throw new ApiException($errorMessage, $request, $response);
        }
        ...
        try {
            ...
            throw new ApiException('Invalid JSON response from API', $request, $response, 0, $e);
        }
    }
    /**
